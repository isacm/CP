...
\begin{code}
render html = do { writeFile "_.html" html ; system "sensible-browser _.html" }
\end{code}
...
main = getArgs >>= cond (not . null) exemp_or_exer errInvArgs
     where
        ...
        exer = cond (((==) 2) . length) execExer errInvArgs
...
execExerPar  = const (putStrLn . show . runEval . (parBTreeMap fib) $ t2)

execExerSeq :: [String] -> IO ()
execExerSeq = const (putStrLn . show . (fmap fib) $ t2)
...
qsplit (n, m) | (m < n) || (n == 0 && m == 0) = i1 ()
              | n < m = let p = n + ( div ((m-n)+1) 2) in i2((p, ((n, p-1), (p+1, m))))
              | otherwise = i2((n, ((n, m-1), (n+1, m))))
...
mgen (l, []) = i1 l
mgen ([], l) = i1 l
mgen = cond (uncurry(<=) . (head >< head)) ( i2 . ( assocr . ( split head tail >< id))) (i2 . (id >< swap) . assocr . ((split head tail)) >< id . swap)
--}
\end{code}
...
hyloTLTree h g = cataTLTree h . anaTLTree g

tipsTLTree = cataTLTree (either singl (conc . (id >< conc)))

invTLTree = cataTLTree (either L  (N . assocr . swap . (id >< swap)))

depthTLTree = fromIntegral . (cataTLTree (either one (succ . (uncurry (max)) . (id >< (uncurry (max))))))


transFirst = (id >< (`div` 2)) >< (-1 +)

transSnd (((x, y), c), n) = (((x + (div c 2), y), div c 2), n - 1)

transTh (((x, y), c), n) = (((x , y + (div c 2)), div c 2), n - 1)

geneSierp :: (Tri, Int) -> Either Tri ((Tri, Int), ((Tri, Int), (Tri, Int)))
geneSierp (t, n) | n == 0 = i1 t
                          | otherwise = i2 (transFirst (t, n), (transSnd (t, n), transTh (t, n)))

geraSierp = curry (anaTLTree geneSierp)
...
rep = finalize . concat . tipsTLTree . (fmap drawTriangle) . uncurry (geraSierp)

type Pir = (Point3D, Side)

type Point3D = (Int, Int, Int)

data PLTree a = Pi a | No (PLTree a, (PLTree a, (PLTree a, (PLTree a, PLTree a)))) deriving (Eq, Show)

inPLTree = either Pi No

outPLTree (Pi a) = i1 a
outPLTree (No(t1,(t2, (t3, (t4, t5))))) = i2 (t1,(t2,(t3, (t4, t5))))

basePLTree g f = g -|- (f >< (f >< (f >< ( f >< f))))

recPLTree f = id -|- (f >< (f >< (f >< (f >< f))))

anaPLTree  g   = inPLTree . recPLTree (anaPLTree g) . g

cataPLTree g   = g . recPLTree (cataPLTree g) . outPLTree

hyloPLTree h g = cataPLTree h . anaPLTree g

instance Functor PLTree
         where fmap f = cataPLTree ( inPLTree . basePLTree f id )

tPir1 ((p, c), n) = let comp = (c `div` 2) in ((p, comp) , n-1)

tPir2 (((x,y,z), c), n) = let comp = (c `div` 2) in (((x + comp, y, z), comp) , n-1)

tPir3 (((x,y,z), c), n) = let comp = (c `div` 2) in (((x, y + comp, z), comp) , n-1)

tPir4 (((x,y,z), c), n) = let comp = (c `div` 2) in (((x, y + comp, z + comp), comp) , n-1)

tPir5 (((x,y,z), c), n) = let comp = (c `div` 2) in (((x + comp, y + comp, z), comp) , n-1)

genePiramide (p, n) | n == 0 = i1 p
                    | otherwise = i2 (tPir1 (p,n), (tPir2 (p,n), (tPir3 (p,n), (tPir4 (p,n), tPir5 (p,n)))))


geraPiramide = curry (anaPLTree genePiramide)

apresentaPiramide = cataPLTree(either singl (conc . (id >< conc) . (id >< (id >< conc)) . (id >< (id >< (id >< conc)))))

psierp t = apresentaPiramide . (geraPiramide t)

drawPyramid :: Pir -> String
drawPyramid ((x,y,z), side)=  undefined
...
rep' = finalize . concat . apresentaPiramide . (fmap drawPyramid) . uncurry (geraPiramide)
...
gene (Right(x,y)) =  D [((x:y),0.95),(y,0.05)]
gene (Left ())     = D [(["stop"],0.90),([],0.1)]
...
parBTreeMap f (Node(r,(d,e))) = do{
    root <- rpar(f r) ;
    esq <- parBTreeMap f d ;
    dir <- parBTreeMap f e ;
    return (Node(root, (esq, dir)))
  }

\end{code}

cp1415 exercicio seq +RTS -s -N4

Total  time  4.05s  ( 2.69s elapsed)

cp1415 exercicio par +RTS -s -N4

Total  time  3.34s  ( 1.34s elapsed)
